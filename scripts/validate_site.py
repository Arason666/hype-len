...
def main() -> None:
    ...
    checks = {
        ...
        "secure paired alert frontend": all(token in javascript for token in ("ALERT_DEFAULT_API_URL", 'credentials: "include"', "submitAlertPairing", '`${url}/pair`', "disconnectAlertDevice")) and "sessionStorage.setItem" not in javascript,
        "mobile first party pairing": 'id="alert-mobile-pair-link"' in html and all(token in alert_service for token in ('path == "/pair"', "_pair_from_form", "HTTPStatus.SEE_OTHER")),
        "multi asset alert controls": 'id="alert-asset"' in html and all(asset in html for asset in ('value="HYPE"', 'value="ETH"', 'value="BTC"')) and "selectedAlertAsset" in javascript,
        "integer alert thresholds": 'id="alert-threshold" type="number" min="0.001" step="0.001"' in html,
        ...
        "always on Hyperliquid price service": all(token in alert_service for token in ("metaAndAssetCtxs", "fetch_mark_prices", "SUPPORTED_ASSETS", "AlertEngine", "ALERT_API_TOKEN")),
        ...
        "UTF-8 ntfy and phone escalation": all(token in alert_service for token in ("send_ntfy", '"title": f"{asset} 到价预警"', "send_phone", "SendCustomAlarmMsg", "phone_events_due")),
        "signed device pairing": all(token in alert_service for token in ("ALERT_SESSION_COOKIE", "issue_session", "verify_session", "HttpOnly; Secure; SameSite=Strict")),

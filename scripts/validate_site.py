...
def main() -> None:
    ...
    checks = {
        ...
        "secure paired alert frontend": all(token in javascript for token in ("ALERT_DEFAULT_API_URL", 'credentials: "include"', '"/api/pair"', "disconnectAlertDevice")) and "sessionStorage.setItem" not in javascript,
        ...
        "UTF-8 ntfy and phone escalation": all(token in alert_service for token in ("send_ntfy", '"title": "HYPE 到价预警"', "send_phone", "SendCustomAlarmMsg", "phone_events_due")),
